...
// #region consts
const EOL = '\n';
const SEPERATOR = `${EOL}<!-- ====================>分隔符<==================== -->${EOL}`; // getTopInfos.js 跟 updateTopInfos.js 保持一致
...
const DIR_MAP = getDirList(BASE_DIR);
...
const allTopInfos = getAllTopInfos();

// 将所有顶部信息写入到 TOP_INFOS.md 文件
fs.writeFileSync(TOP_INFOS_FILE_PATH, allTopInfos.join(''), 'utf8');
console.log(`Generated ${TOP_INFOS_FILE_PATH} successfully.`);
